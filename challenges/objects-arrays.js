...
const lowerCase = zooAnimals.map((notPets) => {
  return notPets.animal_name.toLowerCase();
});
...
*/
const largerPopulation = zooAnimals.filter((notPets) => {
  return notPets.population < 5; //using <= 5 would include pops of 5 or less - instructions wanted less than 5
});
...
*/
const populationTotal = zooAnimals.reduce((population, notPets) => {
  return population += notPets.population;
}, 0);
